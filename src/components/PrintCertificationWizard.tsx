...
function statusColor(s: StepStatus) {
  switch (s) {
    ...
    case "blocked": return "bg-warn-bg text-warn border-warn-edge";
    case "running": return "bg-info-bg text-info border-info-edge";
      ...
      {steps && (
        <div className="mt-4 space-y-2">
          <div className="flex gap-2 text-[11px]">
            <span className={`rounded-full border px-2 py-0.5 ${certified ? "bg-ok-bg text-ok border-ok-edge" : blocked ? "bg-warn-bg text-warn border-warn-edge" : "bg-surface-3"}`}>
              {certified ? "CERTIFIED" : blocked ? "BLOCKED" : "PENDING"}
            </span>
            {requestId && <span className="font-mono text-ink-3">req {requestId.slice(0, 16)}…</span>}
            {jobId && <span className="font-mono text-ink-3">job {jobId}</span>}
          </div>

          <div className="grid gap-2">
            {steps.map((s, idx) => (
              <div key={s.id} className="flex gap-3 rounded-lg border border-edge p-3">
                <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-surface-3 text-[11px] font-bold">{idx + 1}</div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {s.at && <span className="text-[10px] text-ink-4">{new Date(s.at).toLocaleTimeString()}</span>}
                  </div>
                  <div className="mt-1 text-[12px] text-ink-2">{s.message}</div>
                  {s.evidence && <div className="mt-1 font-mono text-[11px] text-ink-3">{s.evidence}</div>}
                  <div className="mt-1 text-[11px] text-ink-4">{s.description}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {timelineUrl && (
            <div className="mt-3">
              <a href={timelineUrl} target="_blank" rel="noopener noreferrer" className="text-xs underline">View Job Timeline (API)</a>
              {jobId && <a href={`/api/jobs/${jobId}/timeline`} className="ml-3 text-xs underline">Timeline JSON</a>}
            </div>
          )}

          <div className="mt-3 rounded bg-warn-bg p-3 text-[11px] text-warn">
