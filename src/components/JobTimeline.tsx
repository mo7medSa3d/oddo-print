...
function stageColor(status: string) {
  switch (status) {
    ...
    case "pending": return "bg-surface-3 text-ink-2 border-edge";
...
export default function JobTimeline({ jobId }: { jobId: string }) {
  ...
  return (
    <div className="space-y-3">
      {correlation && (
        <div className="rounded-lg bg-surface-2 p-3 text-[11px] font-mono">
          <div className="font-semibold">Correlation IDs</div>
          <div className="mt-1 grid grid-cols-1 gap-1 text-ink-2 sm:grid-cols-2">
      ...
      <div className="relative pl-6">
        ...
        {events.map((ev, idx) => (
          <div key={ev.id} className="relative mb-4">
            ...
            <div className="mt-1 flex flex-wrap gap-2 text-[10px] font-mono text-ink-3">
            ...
            {ev.metadata && Object.keys(ev.metadata).length > 0 && (
              <pre className="mt-1 max-h-20 overflow-auto rounded bg-surface-2 p-2 text-[10px]">{JSON.stringify(ev.metadata, null, 2)}</pre>
            )}
          </div>
        ))}
      </div>
