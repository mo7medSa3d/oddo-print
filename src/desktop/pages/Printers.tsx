            ...
            <table className="w-full text-[13px]">
              ...
              <tbody>{rows.map((p) => (
                <tr key={p.id} className="border-b border-edge last:border-0 hover:bg-surface-2/50 transition-colors">
                  ...
                  <td className="px-4 py-3"><span className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-medium border ${p.lifecycle === "retired" ? "bg-surface-2 border-edge text-ink-3" : p.lifecycle === "disabled" ? "bg-warn-bg border-warn-edge text-warn" : "bg-ok-bg border-ok-edge text-ok"}`}>{p.lifecycle || "active"}</span></td>
