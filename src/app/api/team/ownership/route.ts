...
export async function POST(req: Request) {
  ...
  if (!newOwnerId || newOwnerId === claims.userId) return NextResponse.json({ error: "A different member is required" }, { status: 400 });
  ...
  try {
    ...
    const demoted = await tx.update(tenantUsers).set({ role: "admin", updatedAt: new Date() }).where(and(eq(tenantUsers.tenantId, claims.tenantId), eq(tenantUsers.userId, claims.userId), eq(tenantUsers.role, "owner"))).returning({ userId: tenantUsers.userId });
    ...
    await tx.update(managerSessions).set({ revokedAt: new Date() }).where(and(eq(managerSessions.userId, claims.userId), eq(managerSessions.tenantId, claims.tenantId)));
    });
  } catch (error) {
  ...
  void import("../../../../lib/audit").then(({ writeAuditEvent }) => writeAuditEvent({ tenantId: claims.tenantId, actorType: "user", actorId: claims.userId, action: "team.ownership.transferred", resourceType: "user", resourceId: newOwnerId })).catch(() => undefined);
