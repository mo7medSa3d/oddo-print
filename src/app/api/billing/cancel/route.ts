...
export async function POST(req: Request) {
  ...
  try {
    state = await db.transaction(async (tx) => {
      ...
      if (row.billingOperationId) {
        if (
          row.billingOperationType !== "cancel" ||
      ...
      const idempotencyKey = `billing-cancel-${operationId}`;
      await tx.update(tenantSubscriptions)
        .set({
          billingOperationId: operationId,
          billingOperationType: "cancel",
  ...
  try {
    ...
    console.error("billing cancel failed", error instanceof Error ? error.message : "unknown");
  ...
  try {
    ...
    console.error("billing cancel finalization failed", error instanceof Error ? error.message : "unknown");
