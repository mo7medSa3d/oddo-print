      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h1 className="mt-4 flex items-center gap-2.5 text-[28px] font-bold tracking-[-0.04em] text-white leading-tight">
            <CreditCard className="h-6 w-6 text-brand-400" /> Subscriptions
          </h1>
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={s.tenantId} className="hover:bg-white/[0.02] transition">
                  ...
                  <td className="px-5 py-4">
                    {s.status === "active" ? <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-medium text-emerald-300"><CheckCircle className="h-3 w-3" /> Active</span> : s.status === "trialing" ? <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-500/20 bg-brand-500/10 px-2.5 py-1 text-[11px] font-medium text-brand-300"><Clock className="h-3 w-3" /> Trialing</span> : s.status === "past_due" ? <span className="inline-flex items-center gap-1.5 rounded-full border border-red-500/20 bg-red-500/10 px-2.5 py-1 text-[11px] font-medium text-red-300"><AlertTriangle className="h-3 w-3" /> Past Due</span> : <span className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.04] px-2.5 py-1 text-[11px] font-medium text-slate-400">{s.status}</span>}
                  </td>
