            ...
            <tbody className="divide-y divide-white/[0.04]">
              {filtered.map((plan) => (
                <tr key={plan.id} className="hover:bg-white/[0.02] transition">
                  ...
                  <td className="px-5 py-4"><div className="flex flex-wrap gap-1.5"><span className={plan.isActive ? "rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-300" : "rounded-full border border-white/[0.08] bg-white/[0.04] px-2 py-0.5 text-[11px] font-medium text-slate-400"}>{plan.isActive ? "Active" : "Archived"}</span><span className={plan.isPublic ? "inline-flex items-center gap-1 rounded-full border border-brand-500/20 bg-brand-500/10 px-2 py-0.5 text-[11px] font-medium text-brand-300" : "inline-flex items-center gap-1 rounded-full border border-white/[0.08] bg-white/[0.04] px-2 py-0.5 text-[11px] font-medium text-slate-400"}>{plan.isPublic ? <Eye className="h-3 w-3" /> : <EyeOff className="h-3 w-3" />}{plan.isPublic ? "Public" : "Private"}</span></div></td>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-[2px]">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[16px] border border-white/[0.10] bg-[#12151b] shadow-2xl">
        <div className="flex items-start justify-between border-b border-white/[0.06] px-6 py-5">
          <div><p className="text-[11px] font-semibold uppercase tracking-wide text-brand-300">Plan catalog</p><h2 className="mt-1 text-[16px] font-semibold text-white">{isNew ? "Create plan" : "Edit plan"}</h2><p className="mt-1 text-[12px] text-slate-400">Stripe Price ID points to price customers are charged through.</p></div>
...
const INPUT = "w-full rounded-[10px] border border-white/[0.08] bg-[#0b0e13] px-3.5 py-2.5 text-[13px] text-slate-100 placeholder-slate-600 outline-none focus:border-brand-500/30 focus:ring-2 focus:ring-brand-500/15";
