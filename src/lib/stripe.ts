...
export async function stripeRequest(path:string, form:URLSearchParams, idempotencyKey?:string) {
  ...
  const data=await res.json().catch(()=>({})); if(!res.ok) throw new Error(typeof data?.error?.message==="string"?data.error.message:`Stripe request failed (${res.status})`); return data as any;
}
