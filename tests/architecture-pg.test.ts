...
suite("real PostgreSQL runtime architecture gate", () => {
  ...
  it("preserves runtime jobs when agent/printer lifecycle changes", async () => {
    ...
    await pool().query(`INSERT INTO agents (id,tenant_id,name,lifecycle,status) VALUES ('agt_pg','tenant_arch','Agent','active','online')`);
    await pool().query(`INSERT INTO printers (id,tenant_id,agent_id,name,printer_type,device_class,connection_type,protocol,status,lifecycle,config,capabilities)
      VALUES ('prn_pg','tenant_arch','agt_pg','Printer','physical','laser','spooler','spooler','online','active','{}'::jsonb,'{}'::jsonb)`);
    await pool().query(`INSERT INTO print_jobs (id,tenant_id,destination,document_type,agent_id,printer_id,status,payload,expires_at)
      VALUES ('job_pg','tenant_arch','POS','receipt','agt_pg','prn_pg','queued','{"type":"raw","protocol":"raw","encoding":"base64","data":"aA=="}'::jsonb,now()+interval '1 hour')`);
